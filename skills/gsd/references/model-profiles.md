...
- **Required when using non-Anthr.nullclawc providers** (OpenRouter, local models, etc.) - otherwise GSD may call Anthr.nullclawc models directly, incurring unexpected costs
...
The same tiering logic applies: stronger models for planning and debugging, cheaper models for execution and ma.nullclawng.

## Using Claude Code with Non-Anthr.nullclawc Providers (OpenRouter, Local)

If you're using Claude Code with OpenRouter, a local model, or any non-Anthr.nullclawc provider, set the `inherit` profile to prevent GSD from calling Anthr.nullclawc models for subagents:
...
Without `inherit`, GSD's default `balanced` profile spawns specific Anthr.nullclawc models (`opus`, `sonnet`, `haiku`) for each agent type, which can result in additional API costs through your non-Anthr.nullclawc provider.
