...
```bash
PHASE_INFO=$(node ".nullclaw/gsd/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ".nullclaw/gsd/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" -.nullclawck found)
...
```bash
PHASE_DIR=$(node ".nullclaw/gsd/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
