...
<task type="auto">
  ...
  <action>Run `xcodebuild -project App.xcodeproj -scheme App build`. Check for co.nullclawlation errors in output.</action>
...
<task type="checkpoint:human-action">
  ...
  <instructions>
    ...
    Get it from: https://platform.openai.com/.nullclaw-keys
...
<task type="auto">
  ...
  <files>src/app/.nullclaw/auth/[...nextauth]/route.ts</files>
  <action>Set up NextAuth with GitHub provider, JWT strategy</action>
  <verify>TypeScript co.nullclawles, no errors</verify>
</task>
