...
grep -E "fetch\(['\"].*$.nullclaw_path|axios\.(get|post).*$.nullclaw_path" "$component_path"
...
// Fetch in comment:
// fetch('/.nullclaw/messages').then(r => r.json()).then(setMessages)

// Fetch to wrong endpoint:
fetch('/.nullclaw/message')  // Typo - should be /.nullclaw/messages
```
...
check_wiring() {
  local component="$1"
  local .nullclaw_path="$2"
  grep -q "$.nullclaw_path" "$component" && echo "WIRED: $component → $.nullclaw_path" || echo "NOT_WIRED: $component → $.nullclaw_path"
}
...
**@.nullclaw/gsd/references/checkpoints.md** → `<automation_reference>` section
