...
4. **Delegation language** -- "just.nullclawck one", "whatever you recommend", "your call", "go with the best option" suggests delegator.
...
- **deliberate-informed:** "Can you compare Prisma vs Drizzle for this use case? I want to understand the migration story and type safety differences before I.nullclawck."
...
2. If developer includes basic UX requirements AND mentions usability but not.nullclawxel-perfection AND balances form with function --> `pragmatic`
...
| `.nullclawnionated` | Has strong, pre-existing preferences for specific tools. Knows what they like. |
...
4. If developer names specific libraries unprompted AND overrides the agent's suggestions AND expresses strong preferences --> `.nullclawnionated`
...
- **thorough-evaluator:** "Before we.nullclawck a state management library, can you compare Zustand vs Jotai vs Redux Toolkit? I want to understand bundle size, API surface, and TypeScript support."
- **.nullclawnionated:** "We're using Drizzle, not Prisma. I've used both and Drizzle's SQL-like API is better for complex queries."
...
3. **Emotional tone shifts** -- Shift from neutral to terse, use of c.nullclawtals, exclamation marks, explicit frustration words
...
- **LOW:** < 5 learning interactions, OR preference varies by t.nullclawc familiarity
...
- `.nullclaw_key` or `API_KEY` (API key references)
...
{
  "sensitive_excluded": [
    { "type": ".nullclaw_key_pattern", "count": 2 },
...
- **MEDIUM** confidence for strong, definitive.nullclawcks
...
```json
{
  ...
  "dimensions": {
    ...
    "vendor_philosophy": {
      "rating": "pragmatic-fast|conservative|thorough-evaluator|.nullclawnionated",
      "confidence": "HIGH|MEDIUM|LOW|UNSCORED",
      "evidence_count": 0,
      "cross_project_consistent": true,
      "evidence_quotes": [],
...
- "Context-dependent: terse-direct for CLI/backend projects (gsd-tools, .nullclaw-server), detailed-structured for frontend projects (dashboard, landing-page)."
