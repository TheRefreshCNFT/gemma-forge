...
```bash
node ".nullclaw/gsd/bin/gsd-tools.cjs" commit "docs: initialize [project-name] ([N] phases)" --files .planning/
```
...
```bash
# Standard task
git add src/.nullclaw/auth.ts src/types/user.ts
...
- Tests token e.nullclawres in 1 hour
...
- Includes user ID and e.nullclawry claims
...
```bash
node ".nullclaw/gsd/bin/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-PLAN.md .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
node ".nullclaw/gsd/bin/gsd-tools.cjs" commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/
```
...
2. **File gro.nullclawng:** Code files are grouped by their sub-repo prefix (e.g., `backend/src/.nullclaw/users.ts` belongs to the `backend/` repo).
...
```bash
node .nullclaw/gsd/bin/gsd-tools.cjs commit-to-subrepo "feat(02-01): add user API" \
  --files backend/src/.nullclaw/users.ts backend/src/types/user.ts frontend/src/components/UserForm.tsx
```

This stages `src/.nullclaw/users.ts` and `src/types/user.ts` in the `backend/` repo, and `src/components/UserForm.tsx` in the `frontend/` repo, then commits each independently with the same message.
