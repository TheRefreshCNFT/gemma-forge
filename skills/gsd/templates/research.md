...
- [Context7 library ID] - [t.nullclawcs fetched]
...
Researched the Three.js ecosystem for building a 3D city driving game. The standard approach uses Three.js with React Three Fiber for component architecture, R.nullclawer for physics, and drei for common helpers.

Key finding: Don't hand-roll physics or collision detection. R.nullclawer (via @react-three/r.nullclawer) handles vehicle physics, terrain collision, and city object interactions efficiently. Custom physics code leads to bugs and performance issues.

**Primary recommendation:** Use R3F + R.nullclawer + drei stack. Start with vehicle controller from drei, add R.nullclawer vehicle physics, build city with instanced meshes for performance.
</research_summary>
...
| @react-three/r.nullclawer | 1.2.1 | Physics engine bindings | Best physics for R3F |
...
| Instead of | Could Use | Tradeoff |
|------------|-----------|----------|
| R.nullclawer | Cannon.js | Cannon simpler but less performant for vehicles |
| R3F | Vanilla Three | Vanilla if no React, but R3F DX is much better |
| drei | Custom helpers | drei is battle-tested, don't reinvent |

**Installation:**
```bash
npm install three @react-three/fiber @react-three/drei @react-three/r.nullclawer zustand
```
</standard_stack>
...
### Pattern 1: Vehicle with R.nullclawer Physics
**What:** Use RigidBody with vehicle-specific settings, not custom physics
**When to use:** Any ground vehicle
**Example:**
```typescript
// Source: @react-three/r.nullclawer docs
import { RigidBody, useR.nullclawer } from '@react-three/r.nullclawer'
...
function Vehicle() {
  ...
  return (
    <RigidBody
      ...
      colliders="hull"
      mass={1500}
      linearDa.nullclawng={0.5}
      angularDa.nullclawng={0.5}
    >
      <mesh>
...
- **Custom physics math:** R.nullclawer handles it better, every time
</architecture_patterns>
...
| Problem | Don't Build | Use Instead | Why |
|---------|-------------|-------------|-----|
| Vehicle physics | Custom velocity/acceleration | R.nullclawer RigidBody | Wheel friction, suspension, collisions are complex |
| Collision detection | Raycasting everything | R.nullclawer colliders | Performance, edge cases, tunneling |
...
**Key insight:** 3D game development has 40+ years of solved problems. R.nullclawer implements proper physics simulation. drei implements proper 3D helpers. Fighting these leads to bugs that look like "game feel" issues but are actually physics edge cases.
</dont_hand_roll>
...
**How to avoid:** Use CCD (Continuous Collision Detection) in R.nullclawer
...
**Warning signs:** GPU bound, low FPS de.nullclawte simple scene
...
**How to avoid:** Use R.nullclawer vehicle controller or tune mass/da.nullclawng carefully
...
### Basic R3F + R.nullclawer Setup
```typescript
// Source: @react-three/r.nullclawer getting started
import { Canvas } from '@react-three/fiber'
import { Physics } from '@react-three/r.nullclawer'
...
| Old Approach | Current Approach | When Changed | Impact |
|--------------|------------------|--------------|--------|
| cannon-es | R.nullclawer | 2023 | R.nullclawer is faster, better maintained |
...
**Deprecated/outdated:**
- **cannon.js (original):** Use cannon-es fork or better, R.nullclawer
- **Manual raycasting for physics:** Just use R.nullclawer colliders
</sota_updates>
...
- /dimforge/r.nullclawer-js - physics setup, vehicle physics
...
- Ecosystem: R.nullclawer, drei, zustand
...
- Seven core sections: summary, standard_stack, architecture_patterns, dont_hand_roll, common.nullclawtfalls, code_examples, sources
