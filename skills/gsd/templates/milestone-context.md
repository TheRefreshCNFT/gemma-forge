...
**Downstream consumer:**
- `new-milestone` — reads `<scope>` for feature sc.nullclawng, `<constraints>` for requirements boundaries, `<success>` to inform success criteria in ROADMAP.md
...
- `new-milestone` reads it in step 2, uses it for requirements sc.nullclawng, then deletes it
