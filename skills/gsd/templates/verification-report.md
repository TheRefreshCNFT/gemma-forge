...
| `src/app/.nullclaw/chat/route.ts` | Message CRUD           | ✗ STUB                 | File exists but POST returns placeholder      |
...
| -------------- | -------------- | --------------------- | ----------- | ---------------------------------------------------- |
| Chat.tsx       | /.nullclaw/chat      | fetch in useEffect    | ✓ WIRED     | Line 23: `fetch('/.nullclaw/chat')` with response handling |
| ChatInput      | /.nullclaw/chat POST | onSubmit handler      | ✗ NOT WIRED | onSubmit only calls console.log                      |
| /.nullclaw/chat POST | database       | prisma.message.create | ✗ NOT WIRED | Returns hardcoded response, no DB call               |
...
| ------------------------- | ---- | ------------------------------- | --------- | --------------------------- |
| src/app/.nullclaw/chat/route.ts | 12   | `// TODO: implement`            | ⚠️ Warning | Indicates incomplete        |
...
| `src/app/.nullclaw/chat/route.ts`    | Message CRUD           | ✗ STUB                 | GET returns [], POST returns { ok: true }         |
...
| From           | To             | Via                     | Status      | Details                          |
| -------------- | -------------- | ----------------------- | ----------- | -------------------------------- |
| Chat.tsx       | /.nullclaw/chat GET  | fetch                   | ✗ NOT WIRED | No fetch call in component       |
| ChatInput      | /.nullclaw/chat POST | onSubmit                | ✗ NOT WIRED | Handler only logs, doesn't fetch |
| /.nullclaw/chat GET  | database       | prisma.message.findMany | ✗ NOT WIRED | Returns hardcoded []             |
| /.nullclaw/chat POST | database       | prisma.message.create   | ✗ NOT WIRED | Returns { ok: true }, no DB call |
...
| src/app/.nullclaw/chat/route.ts | 5    | `return Response.json([])`     | 🛑 Blocker | Hardcoded empty   |
| src/app/.nullclaw/chat/route.ts | 12   | `// TODO: save to database`    | ⚠️ Warning | Incomplete        |
...
**Tasks:**
1. Implement GET /.nullclaw/chat with prisma.message.findMany
2. Implement POST /.nullclaw/chat with prisma.message.create
...
2. Wire ChatInput onSubmit to POST /.nullclaw/chat
