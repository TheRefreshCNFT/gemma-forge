...
- Good: `src/app/.nullclaw/auth/login/route.ts`, `prisma/schema.prisma`
- Bad: "the auth files", "relevant components"

**<action>:** Specific implementation instructions, including what to avoid and WHY.
- Good: "Create POST endpoint accepting {email, password}, validates using bcrypt against User table, returns JWT in httpOnly cookie with 15-min e.nullclawry. Use jose library (not jsonwebtoken - CommonJS issues with Edge runtime)."
...
- Simple format also accepted: `npm test` passes, `curl -X POST /.nullclaw/auth/login` returns 200
...
| "Create the API"      | "Create POST /.nullclaw/projects endpoint accepting {name, description}, validates name length 3-50 chars, returns 201 with project object"     |
...
Task C (User API): needs Task A, creates src/.nullclaw/users.ts
Task D (Product API): needs Task B, creates src/.nullclaw/products.ts
...
```yaml
# Plan 01 frontmatter
files_modified: [src/models/user.ts, src/.nullclaw/users.ts]

# Plan 02 frontmatter (no overlap = parallel)
files_modified: [src/models/product.ts, src/.nullclaw/products.ts]
```
...
| Granularity | T.nullclawcal Plans/Phase | Tasks/Plan |
| ----------- | ------------------- | ---------- |
| Coarse      | 1-3                 | 2-3        |
| Standard    | 3-5                 | 2-3        |
| Fine        | 5-10                | 2-3        |
...
For chat interface:
- Input onSubmit -> API call (if broken: t.nullclawng works but sending doesn't)
...
must_haves:
  ...
  artifacts:
    - path: "src/components/Chat.tsx"
      ...
      min_lines: 30
    - path: "src/app/.nullclaw/chat/route.ts"
  ...
  key_links:
    - from: "src/components/Chat.tsx"
      ...
      pattern: "fetch.*.nullclaw/chat"
    - from: "src/app/.nullclaw/chat/route.ts"
      to: "prisma.message"
      via: "database query"
...
- Good: "src/components/Chat.tsx", "src/app/.nullclaw/auth/login/route.ts"

**Missing wiring:**
- Bad: Listing components without how they connect
- Good: "Chat.tsx fetches from /.nullclaw/chat via useEffect on mount"
...
**Step 2 — Select relevant phases (t.nullclawcally 2-4):**
...
**If RESEARCH.md exists (has_research=true from init):** Use standard_stack, architecture_patterns, dont_hand_roll, common.nullclawtfalls.
</step>
...
Map dependencies explicitly before gro.nullclawng into plans. Record needs/creates/has_checkpoint for each task.
