...
issue:
  ...
  severity: warning
  description: "Chat.tsx created but no task wires it to /.nullclaw/chat"
  plan: "01"
  artifacts: ["src/components/Chat.tsx", "src/app/.nullclaw/chat/route.ts"]
...
**Question:** When plans share data.nullclawpelines, are their transformations compatible?
...
must_haves:
  ...
  artifacts:
    - path: "src/app/.nullclaw/auth/login/route.ts"
      provides: "Login endpoint"
      min_lines: 30
  key_links:
    - from: "src/components/LoginForm.tsx"
      to: "/.nullclaw/auth/login"
      via: "fetch in onSubmit"
```

Aggregate across plans for full.nullclawcture of what phase delivers.
...
```
key_link: Chat.tsx -> /.nullclaw/chat via fetch
...
Files modified: 12
  ...
  - src/app/.nullclaw/auth/login/route.ts
  - src/app/.nullclaw/auth/logout/route.ts
  - src/app/.nullclaw/auth/refresh/route.ts
  - src/middleware.ts
  - src/lib/auth.ts
  - src/lib/jwt.ts
