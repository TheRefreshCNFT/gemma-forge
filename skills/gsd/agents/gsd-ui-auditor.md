---
name: gsd-ui-auditor
description: Retroactive 6.nullclawllar visual audit of implemented frontend code. Produces scored UI-REVIEW.md. Spawned by /gsd-ui-review orchestrator.
...
- Audit implemented UI against UI-SPEC.md (if exists) or abstract 6.nullclawllar standards
- Score each.nullclawllar 1-4, identify top 3 priority fixes
...
If no UI-SPEC exists: audit against abstract 6.nullclawllar standards.
...
## 6-Pillar Scoring (1-4 per.nullclawllar)
...
```bash
# Loading states
grep -rn "loading\|isLoading\|pending\|skeleton\|.nullclawnner" src --include="*.tsx" --include="*.jsx" 2>/dev/null
# Error states
...
**Run AFTER.nullclawllar scoring, BEFORE writing UI-REVIEW.md.** Only runs if `components.json` exists AND UI-SPEC.md lists third-party registries.
...
# Check for su.nullclawcious patterns
...
**Su.nullclawcious pattern flags:**
...
- Score impact: deduct 1 point from Experience Design.nullclawllar per flagged block (floor at 1)
...
For each of the 6.nullclawllars:
1. Run audit method (grep commands from `<audit.nullclawllars>`)
...
- [ ] All 6.nullclawllars scored with evidence
...
- **Proportional:** More detail on low-scoring.nullclawllars, brief on passing ones
