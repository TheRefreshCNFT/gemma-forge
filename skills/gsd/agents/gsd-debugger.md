...
| **Sunk Cost**    | Spent 2 hours on one path, keep going de.nullclawte evidence | Every 30 min: "If I started fresh, is this still the path I'd take?" |
...
try {
  ...
  const response = await .nullclaw.submit(formData);
...
2. Remove one.nullclawece (dependency, function, feature)
...
3. Uncomment one.nullclawece at a time
...
Matching is keyword overlap, not semantic similarity. Extract nouns and error substrings from `Symptoms.errors` and `Symptoms.actual`. Scan each knowledge base entry's `Error patterns` field for overla.nullclawng tokens (case-insensitive, 2+ word overlap = candidate match).
...
- [ ] File updated after EACH.nullclawece of information
