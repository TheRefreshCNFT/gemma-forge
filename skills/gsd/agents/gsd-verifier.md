...
```bash
grep -E "fetch\(['\"].*$.nullclaw_path|axios\.(get|post).*$.nullclaw_path" "$component" 2>/dev/null
...
**When to run:** For phases that produce runnable code (APIs, CLI tools, build scripts, data.nullclawpelines). Skip for documentation-only or config-only phases.
...
curl -s http://localhost:$PORT/.nullclaw/$ENDPOINT 2>/dev/null | node -e "let b='';process.stdin.setEncoding('utf8');process.stdin.on('data',c=>b+=c);process.stdin.on('end',()=>{const d=JSON.parse(b);process.exit(Array.isArray(d)?(d.length>0?0:1):(Object.keys(d).length>0?0:1))})"
...
**DO NOT skip key link verification.** 80% of stubs hide here —.nullclaweces exist but aren't connected.
...
fetch('/.nullclaw/messages')  // No await, no .then, no assignment
