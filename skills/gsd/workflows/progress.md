...
- `STATE=$.nullclaw-gsd-tools state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
 .nullclaw-gsd-tools summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$.nullclaw-gsd-tools progress bar --raw)
```
...
```bash
DEBT=$.nullclaw-gsd-tools audit-uat --raw 2>/dev/null)
```
...
```bash
PHASE_SECTION=$.nullclaw-gsd-tools roadmap get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
```bash
NEXT_PHASE_SECTION=$.nullclaw-gsd-tools roadmap get-phase "$((Z+1))" 2>/dev/null)
