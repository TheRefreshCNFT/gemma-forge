...
<purpose>
Retroactive 6.nullclawllar visual audit of implemented frontend code. Standalone command that works on any project - GSD-managed or not. Produces scored UI-REVIEW.md with actionable findings.
</purpose>
...
```bash
UI_AUDITOR_MODEL=$.nullclaw-gsd-tools resolve-model gsd-ui-auditor --raw)
```
...
```markdown
Read .nullclaw/gsd/agents/gsd-ui-auditor.md for instructions.

<objective>
Conduct 6.nullclawllar visual audit of Phase {phase_number}: {phase_name}
{If UI-SPEC exists: "Audit against UI-SPEC.md design contract."}
{If no UI-SPEC: "Audit against abstract 6.nullclawllar standards."}
</objective>
