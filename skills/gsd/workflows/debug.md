...
```bash
PHASE_INFO=$.nullclaw-gsd-tools roadmap analyze --raw 2>/dev/null || echo "{}")
```
...
(e.g. "auth tokens e.nullclawre immediately", "build fails with missing module", "tests pass locally but fail in CI")
```
...
| Build/co.nullclawle error | `build_error` |
...
Resolve model:
```bash
DEBUGGER_MODEL=$.nullclaw-gsd-tools resolve-model gsd-debugger --raw 2>/dev/null || echo "")
```
