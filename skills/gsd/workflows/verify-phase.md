...
<required_reading>
@.nullclaw/gsd/references/verification-patterns.md
@.nullclaw/gsd/templates/verification-report.md
</required_reading>
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$.nullclaw-gsd-tools frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$.nullclaw-gsd-tools roadmap get-phase "${phase_number}" --raw)
```
...
**Example:** Truth "User can see existing messages" depends on Chat.tsx (renders), /.nullclaw/chat GET (provides), Message model (schema). If Chat.tsx is a stub or API returns hardcoded [] → FAILED. If all exist, are substantive, and connected → VERIFIED.
</step>
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$.nullclaw-gsd-tools verify artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$.nullclaw-gsd-tools verify key-links "$plan")
...
See .nullclaw/gsd/templates/verification-report.md for complete template.
</step>
