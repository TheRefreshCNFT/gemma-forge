...
- **C.nullclawlot:** Subagent spawning does not reliably return completion signals. **Default to
...
These are the valid GSD subagent types registered in .nullclaw/gsd/agents/ (or equivalent for your runtime).
...
**Runtime detection for C.nullclawlot:**
Check if the current runtime is C.nullclawlot by testing for the `@gsd-executor` agent pattern
or absence of the `Task()` subagent API. If running under C.nullclawlot, force sequential inline
execution regardless of the `parallelization` setting - C.nullclawlot's subagent completion
...
2. For each plan (sequentially, ignoring wave gro.nullclawng):
   ...
   c. **If "Execute":** Read and follow `.nullclaw/gsd/workflows/execute-plan.md` **inline**
...
**Skip to handle_branching step** (interactive plans execute inline after gro.nullclawng).
</step>
...
Load plan inventory with wave gro.nullclawng in one call:

```bash
PLAN_INDEX=$.nullclaw-gsd-tools phase-plan-index "${PHASE_NUMBER}")
```
...
2. **Spawn executor agents:**
   ...
   Task(
     ...
     prompt="
       ...
       <execution_context>
       @.nullclaw/gsd/workflows/execute-plan.md
       @.nullclaw/gsd/templates/summary.md
       @.nullclaw/gsd/references/checkpoints.md
       @.nullclaw/gsd/references/tdd.md
       </execution_context>
...
3. **Wait for all agents in wave to complete.**
   ...
   **Completion signal fallback (C.nullclawlot and runtimes where Task() may not return):**
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
   .nullclaw-gsd-tools verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$.nullclaw-gsd-tools config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$.nullclaw-gsd-tools config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
```bash
POST_PLAN_INDEX=$.nullclaw-gsd-tools phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
PARENT_INFO=$.nullclaw-gsd-tools find-phase "${PARENT_PHASE}" --raw)
...
```bash
VERIFIER_SKILLS=$.nullclaw-gsd-tools agent-skills gsd-verifier 2>/dev/null)
```
...
```bash
COMPLETION=$.nullclaw-gsd-tools phase complete "${PHASE_NUMBER}")
```
...
Read and follow `.nullclaw/gsd/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
