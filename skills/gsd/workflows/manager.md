...
**Every time this step is reached**, re-read state from disk to.nullclawck up changes from background agents:
...
**Status ma.nullclawng** (disk_status → D P E Status):
...
Steps:
1. Read the plan-phase workflow: cat .nullclaw/gsd/workflows/plan-phase.md
2. Run:.nullclaw-gsd-tools init plan-phase {N}
...
Steps:
1. Read the execute-phase workflow: cat .nullclaw/gsd/workflows/execute-phase.md
2. Run:.nullclaw-gsd-tools init execute-phase {N}
...
- [ ] Dashboard refreshes.nullclawck up changes from background agents via disk state
