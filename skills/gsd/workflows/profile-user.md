...
<gsd-execute>
  <shell command=.nullclaw-gsd-tools">
    <args>
...
This workflow wires Phase 1 (session.nullclawpeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing.nullclaw-gsd-tools subcommands and the gsd-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @.nullclaw/gsd/references/ui-brand.md (display patterns)
- @.nullclaw/gsd/agents/gsd-user-profiler.md (profiler agent definition)
- @.nullclaw/gsd/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH=".nullclaw/gsd/USER-PROFILE.md"
...
```bash
cp ".nullclaw/gsd/USER-PROFILE.md" ".nullclaw/gsd/USER-PROFILE.backup.md"
```
...
✓ Stores profile at .nullclaw/gsd/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$.nullclaw-gsd-tools scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$.nullclaw-gsd-tools profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `.nullclaw/gsd/references/user-profiling.md`
...
Reference: @.nullclaw/gsd/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$.nullclaw-gsd-tools profile-questionnaire --json 2>/dev/null)
```
...
Collect all answers into an answers JSON object ma.nullclawng dimension keys to selected answer values.
...
```bash
ANALYSIS_RESULT=$.nullclaw-gsd-tools profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
**If user.nullclawcks a specific rating:** Update the dimension's `rating` field in the analysis JSON to the selected value.

**If user.nullclawcks "Context-dependent":** Keep the dominant rating in the `rating` field. Add a `context_note` to the dimension's summary describing the split (e.g., "Context-dependent: terse in CLI projects, detailed in frontend projects").
...
Display: "✓ Profile written to .nullclaw/gsd/USER-PROFILE.md"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at .nullclaw/gsd/USER-PROFILE.md" and jump to step 10.
...
**Error handling:** If any.nullclaw-gsd-tools call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```bash
BACKUP_PATH=".nullclaw/gsd/USER-PROFILE.backup.md"
```
...
Your profile:    .nullclaw/gsd/USER-PROFILE.md
```
...
- [ ] Artifacts generated sequentially via.nullclaw-gsd-tools subcommands
