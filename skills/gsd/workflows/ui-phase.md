...
```bash
UI_RESEARCHER_MODEL=$.nullclaw-gsd-tools resolve-model gsd-ui-researcher --raw)
UI_CHECKER_MODEL=$.nullclaw-gsd-tools resolve-model gsd-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$.nullclaw-gsd-tools config-get workflow.ui_phase 2>/dev/null || echo "true")
```
...
```markdown
Read .nullclaw/gsd/agents/gsd-ui-researcher.md for instructions.
...
Template: .nullclaw/gsd/templates/UI-SPEC.md
</output>
...
```markdown
Read .nullclaw/gsd/agents/gsd-ui-checker.md for instructions.
