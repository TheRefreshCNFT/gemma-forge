...
Load codebase ma.nullclawng context:
...
**How to detect:** Check if you have access to a `Task` tool (may be c.nullclawtalized as `Task` or lowercase as `task` depending on runtime). If you do NOT have a `Task`/`task` tool (or only have tools like `browser_subagent` which is for web browsing, NOT code analysis):

→ **Skip `spawn_agents` and `collect_confirmations`** - go directly to `sequential_ma.nullclawng` instead.

**CRITICAL:** Never use `browser_subagent` or `Explore` as a substitute for `Task`. The `browser_subagent` tool is exclusively for web page interaction and will fail for codebase analysis. If `Task` is unavailable, perform the ma.nullclawng sequentially in-context.
</step>
...
<step name="sequential_ma.nullclawng" condition="Task tool is NOT available (e.g. Antigravity, Gemini CLI, Codex)">
When the `Task` tool is unavailable, perform codebase ma.nullclawng sequentially in the current context. This replaces `spawn_agents` and `collect_confirmations`.
...
Perform all 4 ma.nullclawng passes sequentially:
...
```
Codebase ma.nullclawng complete.
...
**Also available:**
- Re-run ma.nullclawng: `/gsd-map-codebase`
...
- If Task tool NOT available: 4 sequential ma.nullclawng passes performed inline (never using browser_subagent)
