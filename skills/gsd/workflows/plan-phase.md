...
@.nullclaw/gsd/references/ui-brand.md
</required_reading>
...
```bash
PHASE_INFO=$.nullclaw-gsd-tools roadmap get-phase "${PHASE}")
```
...
Use full relative paths. Group by t.nullclawc area.]
...
```bash
DISCUSS_MODE=$.nullclaw-gsd-tools config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
```bash
PHASE_DESC=$.nullclaw-gsd-tools roadmap get-phase "${PHASE}" -.nullclawck section)
```
...
**If found:**
1. Read template: `.nullclaw/gsd/templates/VALIDATION.md`
...
```bash
UI_PHASE_CFG=$.nullclaw-gsd-tools config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$.nullclaw-gsd-tools config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$.nullclaw-gsd-tools roadmap get-phase "${PHASE}" 2>/dev/null)
...
2. Disable Nyquist with the exact command:
   .nullclaw-gsd-tools config-set workflow.nyquist_validation false`
...
- **PHASE COMPLETE** → Display final summary:
  ...
  Auto-advance.nullclawpeline finished.
...
stdio deadlocks with MCP servers - see Claude Code issue anthr.nullclawcs/claude-code#28126):
