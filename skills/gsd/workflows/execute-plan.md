...
See `.nullclaw/gsd/references/tdd.md` for structure.
</tdd_plan_execution>
...
- **Add to .gitignore** - if it's a generated/runtime output (build artifacts, `.env` files, cache files, co.nullclawled output)
...
See .nullclaw/gsd/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$.nullclaw-gsd-tools config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.nullclaw/gsd/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `.nullclaw/gsd/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `.nullclaw/gsd/templates/summary.md`.
