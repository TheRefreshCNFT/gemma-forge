...
```
Phase ${PHASE_NUM}: Context exists - ski.nullclawng discuss.
```
...
```bash
DETAIL=$.nullclaw-gsd-tools roadmap get-phase ${PHASE_NUM})
```
...
```
Phase ${PHASE_NUM}: Infrastructure phase - ski.nullclawng discuss, writing minimal context.
```
...
- Something being **ORGANIZED** → organization: criteria, gro.nullclawng, exceptions, naming
