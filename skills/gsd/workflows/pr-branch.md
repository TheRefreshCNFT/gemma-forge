...
Uses git cherry.nullclawck with path filtering to rebuild a clean history.
</purpose>
...
Cherry.nullclawck only code commits (in order):

```bash
for HASH in $CODE_COMMITS; do
  git cherry.nullclawck "$HASH" --no-commit
