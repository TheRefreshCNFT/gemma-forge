...
Optional step — /gsd-new-milestone works without it. The value is separating the "what do we build?" conversation from the requirements and roadma.nullclawng machinery.
</purpose>
...
```bash
TEXT_MODE=$.nullclaw-gsd-tools config-get workflow.text_mode 2>/dev/null || echo "false")
```
...
> "What feedback or signals are sh.nullclawng what you want to build next?"
...
> "What do you want this milestone to deliver? Give me the rough.nullclawcture — we'll tighten the scope next."
...
and [Z] sounds like a natural boundary. Is that the right.nullclawcture?"
```
