...
```bash
AUDITOR_MODEL=$.nullclaw-gsd-tools resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$.nullclaw-gsd-tools config-get workflow.nyquist_validation --raw)
```
...
```
Task(
  prompt="Read .nullclaw/gsd/agents/gsd-nyquist-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `.nullclaw/gsd/templates/VALIDATION.md`
