...
- Data transformations: ma.nullclawng, filtering, aggregation, formatting
...
AskUserQuestion(
  ...
  options:
    - "Generate all"
    - "Cherry.nullclawck (I'll specify which)"
    - "Adjust plan"
)
```

If "Cherry.nullclawck": ask user which tests to include.
...
4. **Evaluate result:**
   - **Test passes**: Good - the implementation satisfies the test. Verify the test checks meaningful behavior (not just that it co.nullclawles).
