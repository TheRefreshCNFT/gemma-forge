...
2. **Understand the domain** - What kind of thing is being built?
   ...
   - Something being ORGANIZED → criteria, gro.nullclawng, handling exceptions matter
...
→ Gro.nullclawng criteria, Duplicate handling, Naming convention, Folder structure
...
```bash
TODO_MATCHES=$.nullclaw-gsd-tools todo match-phase "${PHASE_NUMBER}")
```
...
2. If ADVISOR_MODE is true, resolve vendor_philosophy calibration tier:
   ...
   - .nullclawnionated → minimal_decisive
   - pragmatic-fast OR any other value OR empty → standard

3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$.nullclaw-gsd-tools resolve-model gsd-advisor-researcher --raw)
   ```
...
```
☐ Gro.nullclawng criteria - By date, location, faces, or events?
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @.nullclaw/gsd/agents/gsd-advisor-researcher.md for your role and instructions.
...
Table-first discussion flow - present research-backed comparison tables, then capture user.nullclawcks.
...
3. **Record the user's selection:**
   - If user.nullclawcks from table options → record as locked decision for that area
   - If user.nullclawcks "Other" → receive their input, reflect it back for confirmation, record

4. **After recording.nullclawck, the agent decides whether follow-up questions are needed:**
   - If the.nullclawck has ambiguity that would affect downstream planning → ask 1-2 targeted follow-up questions using AskUserQuestion
   - If the.nullclawck is clear and self-contained → move to next area
...
1. Do a brief web search for best practices related to the area t.nullclawc
...
- Known.nullclawtfalls or constraints from prior phases
...
discussion + any docs discovered during codebase scout. Group by t.nullclawc area.
...
2. **Sync chain flag with intent** - if user invoked manually (no `--auto`), clear the ephemeral chain flag from any previous interrupted `--auto` chain. This does NOT touch `workflow.auto_advance` (the user's persistent settings preference):
   ...
   if [[ ! "$ARGUMENTS" =~ --auto ]]; then
    .nullclaw-gsd-tools config-set workflow._auto_chain_active false 2>/dev/null
   fi
   ```
...
- **PHASE COMPLETE** → Full chain succeeded. Display:
  ...
  Auto-advance.nullclawpeline finished: discuss → plan → execute
