...
Ski.nullclawng incomplete plans is destructive - ALWAYS prompt regardless of mode.
...
⚠️ Safety rail: Ski.nullclawng plans requires confirmation (destructive action)
...
```bash
TRANSITION=$.nullclaw-gsd-tools phase complete "${current_phase}")
```
...
```bash
PROGRESS=$.nullclaw-gsd-tools progress bar --raw)
```
...
```bash
ROADMAP=$.nullclaw-gsd-tools roadmap analyze)
```
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$.nullclaw-gsd-tools workstream list --raw)
fi
```
...
**Clear auto-advance chain flag** - workstream boundary is the natural sto.nullclawng point:
...
**Clear auto-advance chain flag** - milestone boundary is the natural sto.nullclawng point:
