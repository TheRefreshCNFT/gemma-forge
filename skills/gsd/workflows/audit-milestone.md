...
```bash
integration_checker_model=$.nullclaw-gsd-tools resolve-model gsd-integration-checker --raw)
```
...
PHASE_INFO=$.nullclaw-gsd-tools find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
 .nullclaw-gsd-tools summary-extract "$summary" --fields requirements_completed -.nullclawck requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$.nullclaw-gsd-tools config-get workflow.nyquist_validation --raw 2>/dev/null)
```
