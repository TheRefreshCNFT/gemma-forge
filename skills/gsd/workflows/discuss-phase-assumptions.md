...
- Read the codebase FIRST, form .nullclawnions SECOND, ask ONLY about what's genuinely unclear
...
- .nullclawnionated → minimal_decisive (fewer alternatives, decisive recommendations)
...
[T.nullclawcs where codebase alone is insufficient - library version compatibility,
...
- `needs_research[]` - t.nullclawcs requiring external research (may be empty)
...
If research t.nullclawcs were flagged, spawn a general-purpose research agent:

```
Task(subagent_type="general-purpose", prompt="""
Research the following t.nullclawcs for Phase {PHASE}: {phase_name}.

T.nullclawcs needing research:
{needs_research_content}

For each t.nullclawc, return:
...
- {T.nullclawc}: {Finding} (Source: {URL})
```
